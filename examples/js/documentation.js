function startCanvas(l){
    ...
    var badgeUp=new Canvas2d.Stage('badgeup',160,40);
    var badgeDw=new Canvas2d.Stage('badgedw',160,40);
    //set variables
    ...
    xqr.send();
    //parse data Examples
    var CLO=/^\s{4}(\w+)\:\sfunction\(c\,\sw\,\sh\,?\s?params?\)\s\{/,CLS=/^(Stage|Sprite|DisplayObjects|Tweener|Colors)\.prototype\.(\w+)\s\=?\s(function\(c\,\sw\,\sh\,?\s?params?\)\s\{)?/,VF=/^\}\;/,UF=/^\s{4}\},?/,codeObj={Stage:{},Sprite:{},DisplayObjects:{},Tweener:{},Colors:{}}; 
    ...
    //scroll handlers
    hook.rectRound(-4,0,8,40,5,'lightgray');hook.x=10;hook.y=0;
    ...
    //set background's element for badgeUp
    var bLayerUp,bBackUp;
    bLayerUp=new Canvas2d.Sprite('badgelayerup');bBackUp=new Canvas2d.DisplayObjects('badgupbackground');
    bBackUp.shape(0,0,[{moveTo:[0,0]},{lineTo:[0,20]},{quadraticCurveTo:[2,28,10,30]},{lineTo:[130,30]},{quadraticCurveTo:[138,28,140,20]},{lineTo:[140,0]},{lineTo:[0,0]}],'white');
    bBackUp.shadow={color:'#CCCCCC',offsetX:0,offsetY:0,blur:10};bBackUp.x=10;
    bLayerUp.add(bBackUp);
    badgeUp.add(bLayerUp);badgeUp.draw();
    ...
    bBackDw.shape(0,0,[{moveTo:[10,0]},{lineTo:[130,0]},{quadraticCurveTo:[138,2,140,10]},{lineTo:[140,300]},{lineTo:[0,300]},{lineTo:[0,10]},{quadraticCurveTo:[2,2,10,0]}],'white');bBackDw.y=10;bBackDw.x=10;bBackDw.shadow={color:'#CCCCCC',offsetX:0,offsetY:0,blur:10};
    ...
    for(i=0;i<proList.length;i++){
        ...
        dClass.text(proList[i],0,0,'bold',26,font,'black',null,'center','middle');dClass.paddingLeft=dClass.paddingTop=5;
    ...
    menuBadge=new Canvas2d.DisplayObjects('backgroundmenumoving',false);menuBadge.rectRound(0,-(dClass.height/2),dClass.width+10,30,12,'#AAAAAA');sConstr.add(menuBadge);menuBadge.x=dClass.x-(dClass.width/2)-5;menuBadge.y=dClass.y;
    ...
    function constrOpen(e){
        e.target.lineColor=null;mainDocs.container.style.cursor='default';
    ...
    function pushExample(c,o){
        examples.innerHTML='';codePre.innerHTML='';cleanLayer(bLayerDw,tween);scaleMultyOptions(160,40);
        if(c in codeObj){
            if(o in codeObj[c]){
    ...
    function pushMultyOptions(c,o){
        ...
        scaleMultyOptions(160,parseInt(bLayerDw.getBound().height));
    }
    function scaleMultyOptions(w,h){
        if(badgeDw.height===h){return;}
        xx=badgeDw.height;t=0;yy=h-xx;
        console.log(w,h);
        tween.addTweener(bBackDw,{y:10,duratione:1000,ease:'easeOutQuint',onTween:endRedraw,data:{w:w,h:yy,c:xx}});
    }
    function endRedraw(e){
        t+=badgeDw.getTimeInterval();
        x=tween.easeOutQuint(t,e.data.c,e.data.h,1000);
        //console.log(x,t,e.data.x,e.data.h);
        badgeDw.resize(e.data.w,x);bLayerDw.draw();
    }
    function pushMultyExample(e){
        examples.innerHTML='';codePre.innerHTML='';
        tempExp=eval(e.target.content.c);tempExp=new tempExp();tempExp[e.target.content.o][e.target.content.z]('examples',width,300,[location]);
    ...
    function scrollVisible(e){
        ...
        if(idx.height>refDocs.height){
            ...
            tween.addTweener(hook,{color:'white',duration:500,ease:'easeSine'});
        }
    }
    ...
    function endMenuBadge(e){
        tween.addTweener(menuBadge,{alpha:0,duration:300,ease:'easeOutQuint'});
    }
    function dcOver(e){
        mainDocs.container.style.cursor='pointer';
        e.target.color='black';
        e.target.parent.draw();
        tween.addTweener(menuBadge,{alpha:1,x:e.target.x-(e.target.width/2)-5,width:e.target.width+10,duration:800,ease:'easeInOutQuint',onEnd:null});
    }
    function dcOut(e){
        mainDocs.container.style.cursor='default';
        e.target.color='#EEEEEE';
        e.target.parent.draw();
    }
    function coOver(e){
        mainDocs.container.style.cursor='pointer';
        e.target.lineColor=e.target.content.lineColor;
        e.target.parent.draw();
    }
    function coOut(e){
        mainDocs.container.style.cursor='default';
        e.target.lineColor=null;
        e.target.parent.draw();
    }
}